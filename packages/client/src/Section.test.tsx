...
describe('Section', () => {
  it('renders the title', () => {
    render(<Section title="CANDLES"><div>content</div></Section>)
    expect(screen.getByText('CANDLES')).toBeInTheDocument()
  })

  it('shows children by default', () => {
    render(<Section title="TEST"><div>content</div></Section>)
  ...
  it('starts closed when defaultOpen=false', () => {
    render(<Section title="TEST" defaultOpen={false}><div>content</div></Section>)
    expect(screen.queryByText('content')).not.toBeInTheDocument()
  })
